...
-- Admin-only: total messages SENT per user, for the /admin/users dashboard.
-- Grouped aggregate so the dashboard fetches one row per sender instead of paging
-- the whole messages table. security definer + revoke-from-public so it is only
-- reachable by the service role (the admin API), never anon/authenticated.
