...
-- Returns the new match id so the client can jump straight into the conversation
-- instead of waiting for a reload. The return type changed (void -> uuid), which
-- `create or replace` can't do, so drop first to keep this file re-runnable.
...
-- Nearby invitations
--
-- DHs the user just saw on the map reach out with a pending match_request + an
-- opener; it only becomes a match when the user accepts (rpc_accept_match_request
-- above seeds the conversation with the opener).
--
...
--
-- The scheduled_dh_invites queue table + nearby config keys live in schema.sql.
-- ============================================================================

-- Scheduler — called by the find-nearby route with the DHs it just showed. Caps
-- each user to max_invites_per_day actual invitations in any rolling 24h.
...
-- Claim due rows atomically (the dispatcher generates the opener outside any lock).
...
-- Stats for the Me-page popularity tiers + pending invitation count.
...
-- Grants for the nearby-invite functions (scheduler/claimer are service-role only).
