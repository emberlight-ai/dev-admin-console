...
values
  ...
  ('selfie_cooldown_hours', '3', 'DEPRECATED 2026-06-07 — no longer read by dh-auto-reply. Replaced by selfie_cooldown_minutes.'),
  ('selfie_cooldown_minutes', '30', 'Min minutes between spontaneous (intimacy-driven) selfies a DH sends within one conversation. Replaces selfie_cooldown_hours.'),
  ('enable_selfie_reciprocation', 'true', 'When the user sends a photo/selfie, let the DH reply with one of its own (bypasses the intimacy threshold).'),
  ('selfie_reciprocate_gap_minutes', '2', 'Min minutes between selfies on the strong-cue path (user sent a pic or asked to see them).'),
  ('selfie_tease_intimacy_threshold', '45', 'Min intimacy score (0-100) where DH selfies move from casual to tease.'),
  ('selfie_reward_intimacy_threshold', '75', 'Min intimacy score (0-100) where DH selfies move from tease to reward.'),
