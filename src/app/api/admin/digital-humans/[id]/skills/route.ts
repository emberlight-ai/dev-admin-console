...
/**
 * GET /api/admin/digital-humans/[id]/skills
 * → { all: [{key,name,description,sort_order}], selected: string[] }
 */
export async function GET(
  ...
  const [catalogRes, selectedRes] = await Promise.all([
    supabaseAdmin
      ...
      .eq('active', true)
      .order('sort_order'),
    supabaseAdmin.from('dh_skills').select('skill_key').eq('user_id', id),
  ]);
  ...
  return NextResponse.json({
    all: catalogRes.data ?? [],
    selected: (selectedRes.data ?? []).map((r) => r.skill_key),
  });
}

/** PUT /api/admin/digital-humans/[id]/skills  { keys: string[] } — replace-all. */
export async function PUT(
  ...
  const wanted = [...new Set(rawKeys.filter((k): k is string => typeof k === 'string').map((k) => k.trim()))];
  ...
  const { error: delErr } = await supabaseAdmin.from('dh_skills').delete().eq('user_id', id);
  if (delErr) return jsonError(delErr.message, 500);
  if (keys.length > 0) {
    const { error: insErr } = await supabaseAdmin
      .from('dh_skills')
      .insert(keys.map((key) => ({ user_id: id, skill_key: key })));
