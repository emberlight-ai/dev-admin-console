...
export async function POST(req: Request, { params }: { params: Promise<{ userid: string }> }) {
  try {
    ...
    const productId = plan_type === 'monthly' ? 'amber.premium.monthly.0.0' : 'amber.premium.yearly.0.0'
    ...
    if (catalogError || !catalogData) {
      throw new Error(`Could not find catalog row for ${productId}: ${catalogError?.message}`)
    }
