...
/**
 * The digital-human detail screen. Lives beside the route rather than in it so
 * Creator Studio can render it for a fixed expert — Next validates a page
 * module's props, and `expertId` is not one of them.
 *
 * `expertId` overrides the URL id; left undefined on the real route.
 */
export function DigitalHumanDetail({ expertId }: { expertId?: string } = {}) {
  const routeParams = useParams();
  const id = expertId ?? routeParams?.id;
  ...
  return (
    <div className="max-w-6xl space-y-4">
      {/* Studio embeds this page for a single fixed expert, where a link back
          to the operator list would eject the creator out of their own product. */}
      {!expertId && (
